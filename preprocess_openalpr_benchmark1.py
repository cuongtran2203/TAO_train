...
def parse_args(args=None):
    ...
    parser.add_argument(
        ...
        default="/home/vdungx/Desktop/train_tao_cuongcv/lprnet/img_2/img/"
    )
...
def prepare_data(input_dir, img_list, output_dir):
    ...
    for img_name in img_list:
        ...
        with open(label_path, "r+") as f,open(os.path.join(target_label_path,img_name.split(".")[0] + ".txt"), "w") as out:
            ...
            #print(label_items)
            if label_lines =='':
                print("txt emtry",label_path)
            if len(label_items) != 0:
                lp=label_items[0]
                lp1="".join(c for c in lp if c.isalpha() or c.isalnum())
                
                if len(lp1)<10 and len(lp1)>1:
